...
export const ImageOptimizationItem = memo(({ item }: Props) => {
  ...
  return (
    <div className={cn('flex flex-col', isDisabled ? 'opacity-50 cursor-not-allowed' : '')}>
      ...
      {isOpen ? (
        <ImageOptimizationLevel optimizationPercent={optimizationPercent} handleOnOptimizationLevel={handleOnOptimizationLevel} />
      ) : null}
    </div>
  )
})
